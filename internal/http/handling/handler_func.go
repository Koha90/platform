...
func createInvokedhandlerFunc(ctx context.Context, routes []Route) templates.InvokeHandleFunc {
		...
		for _, route := range routes {
			if strings.EqualFold(handlerName, route.handlerName) &&
				...
				if action, ok := result[0].Interface().(*actionresults.TemplateActionResult); ok {
					invoker := createInvokedhandlerFunc(ctx, routes)

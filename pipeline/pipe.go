...
func CreatePipeline(components ...interface{}) RequestPipeline {
	...
	for i := len(components) - 1; i >= 0; i-- {
		...
		if servComp, ok := currentComponent.(ServiceMiddlewareComponent); ok {
			f = createServiceDependetFunction(currentComponent, nextFunc)
			servComp.Init()
...
func createServiceDependetFunction(component interface{}, nextFunc RequestPipeline) RequestPipeline {
	...
	if method.IsValid() {
		...
		panic("No ProcessRequestWithServices method define")
	}
}

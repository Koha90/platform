...
var (
	trace = fmt.Sprintf("%c[1;46;34m%s%c[0m", 0x1B, "[TRACE]: ", 0x1B)
	debug = fmt.Sprintf("%c[1;47;34m%s%c[0m", 0x1B, "[DEBUG]: ", 0x1B)
	info  = fmt.Sprintf("%c[1;40;34m%s%c[0m", 0x1B, "[INFO]: ", 0x1B)
	warn  = fmt.Sprintf("%c[1;43;34m%s%c[0m", 0x1B, "[WARN]: ", 0x1B)
	fatal = fmt.Sprintf("%c[1;45;34m%s%c[0m", 0x1B, "[FATAL]: ", 0x1B)
)
